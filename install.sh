...
post_install() {
    ...
    case "$PROFILE" in
        ...
        browser)
            ...
            ;;
        iot)
            echo "    # 1. Configure WiFi credentials (choose your target):"
        echo "    # 1. Build firmware (Docker — no local ESP-IDF needed):"
        echo "    # Supported targets: esp32s3, esp32c3, esp32c6"
            echo "    cd firmware/esp32-csi-node"
        echo "    ./build.sh esp32c3"
            echo ""
        echo "    # 2. Flash to ESP32 (replace COM7 and --chip):"
        echo "    # S3: python -m esptool --chip esp32s3 --port COM7 write_flash 0x0 build/bootloader/bootloader.bin 0x8000 build/partition_table/partition-table.bin 0x10000 build/esp32-csi-node.bin"
        echo "    # C3: python -m esptool --chip esp32c3 --port COM7 write_flash 0x0 build/bootloader/bootloader.bin 0x8000 build/partition_table/partition-table.bin 0x10000 build/esp32-csi-node.bin"
            echo ""
